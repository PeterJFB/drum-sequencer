...
/**
 * The {@link Track} class represents a group of instruments programmed to a specific pattern, and
 * is also used to edit the pattern of these instruments.
 */
public class Track {
   ...
   * instruments, trackName and artistName later.
   */
  public Track() {}
   ...
   *
   * @return a List of instrument names as {@link String}s. The reason this is a List and not a
   *         Collection is so that when saving tracks, the order in which you added the instruments
   *         is kept in place, which makes for a better user experience when saving and loading
   *         tracks.
   */
   ...
   * @return a copy of the pattern for the given instrument, or null if the instrument is not in the
   *         track
   */
  ...
  public void setTrackName(String trackName) throws IllegalArgumentException {
    if (trackName != null && trackName.length() > TRACK_NAME_LENGTH) {
      throw new IllegalArgumentException(
          "Track name cannot be more than %s characters long. Found %s".formatted(TRACK_NAME_LENGTH,
              trackName.length()));
    }
  ...
  public void setArtistName(String artistName) throws IllegalArgumentException {
    if (artistName != null && artistName.length() > ARTIST_NAME_LENGTH) {
      throw new IllegalArgumentException(
          "Artist name cannot be more than %s characters long. Found %s"
  ...
  /**
   * Adds another instrument to the track with length TRACK_LENGTH and sets all values/sixteenths in
   * the pattern to be inactive (false).
   *
  ...
  /**
   * Toggles the value/sixteenth in the pattern of the given instrument (from true to false, or vise
   * versa).
   *
   ...
   */
  public void toggleSixteenth(String instrumentName, int sixteenthIndex)
    ...
    if (0 > sixteenthIndex || sixteenthIndex >= TRACK_LENGTH) {
      throw new IllegalArgumentException(
          "Cannot update instrument. Sixteenth index is outside the track bounds [0-%s): %s"
              .formatted(TRACK_LENGTH, sixteenthIndex));
    }
